            ...
            while ($row_res=$rqt->fetch()){  ?>
                ...
                <tr class="gradeX">
                   ...
                   <?php }
                    ...
                    <?php }
                    if($row_res['lib_mpay']==17){
                        ?>
                        <td><?php echo"Augmentation de duree";?></td>
                    <?php }
                    ...
                    <td>&nbsp;
                        ...
                        <?php }
                        if($row_res['lib_mpay']==17){ ?>
                            <a href="sortie/g-avenantaug/<?php echo crypte($row_res['cod_av']) ?>" onClick="window.open(this.href, 'Devis', 'height=600, width=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no'); return(false);" title="Imprimer"><i CLASS="icon-print icon-2x" style="color:#0e90d2"/></a>
                        <?php }
                        ?>
                    </td>
                </tr>
...
<script language="JavaScript">
    ...
    function fpageapconso(page,nbpage,code){
        if(page >=0){
            ...
            }else{$("#content").load('adm/apolvoyind.php?page='+page+'&code='+code);}
