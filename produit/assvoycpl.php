...
`devisw` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='1' AND s.`id_user`='$id_user' AND $condition AND d.`cod_formul`='3' ORDER BY d.`cod_dev` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
...
`devisw` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='1' AND s.`id_user`='$id_user' AND $condition AND d.`cod_formul`='3' ORDER BY d.`cod_dev` DESC LIMIT $part ,7");
    $rqt->execute();
            ...
            while ($row_res=$rqt->fetch()){  ?>
                ...
                <tr class="gradeX">
                    ...
                    <td>&nbsp;
                        ...
                        <?php if($row_res['bool']==0){ ?>
                            <a onClick="vdevoy('<?php echo $row_res['cod_dev'];?>','<?php echo $row_res['dat_eff'];?>','assvoycpl.php')" title="Valider"><img  src="img/icons/ok.png"/></a>&nbsp;&nbsp;&nbsp;
                        <?php } ?>
                        <a onClick="ddev('prod','assvoycpl.php','<?php echo $row_res['cod_dev'];?>')" title="Supprimer"><img  src="img/icons/supp.png"/></a>
                    </td>
                </tr>
            <?php } ?>
            </tbody>
