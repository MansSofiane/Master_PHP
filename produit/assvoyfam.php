...
`devisw` as d,`souscripteurw` as s, `periode` as p WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='1'  AND s.`id_user`='$id_user' AND $condition AND p.`cod_per`=d.`cod_per` AND d.`cod_formul`='4' ORDER BY d.`cod_dev` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
...
`devisw` as d,`souscripteurw` as s, `periode` as p WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='1'  AND s.`id_user`='$id_user' AND $condition AND p.`cod_per`=d.`cod_per` AND d.`cod_formul`='4' ORDER BY d.`cod_dev` DESC LIMIT $part ,7");
    $rqt->execute();
                  ...
                  <?php }else { ?>
                    ...
                    <td>&nbsp;
                        ...
                        <a href="sortie/d-famille/<?php echo crypte($row_res['cod_dev']) ?>" onClick="window.open(this.href, 'DevisF', 'height=600, width=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no'); return(false);" title="Imprimer"><i CLASS="icon-print icon-2x" style="color:#0e90d2"/></a>&nbsp;&nbsp;&nbsp;
                            ...
                            <a onClick="vdevoy('<?php echo $row_res['cod_dev'];?>','<?php echo $row_res['dat_eff'];?>','assvoyfam.php')" title="Valider"><img  src="img/icons/ok.png"/></a>&nbsp;&nbsp;&nbsp;

                        <a onClick="ddev('prod','assvoyfam.php','<?php echo $row_res['cod_dev'];?>')" title="Supprimer"><img  src="img/icons/supp.png"/></a>
                    </td>
                </tr>
            <?php } ?>
            </tbody>
