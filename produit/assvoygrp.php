...
`devisw` as d,`souscripteurw` as s, `periode` as p WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='1'  AND s.`id_user`='$id_user' AND $condition AND p.`cod_per`=d.`cod_per` AND d.`cod_formul`='5' ORDER BY d.`cod_dev` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
...
`devisw` as d,`souscripteurw` as s, `periode` as p WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='1'  AND s.`id_user`='$id_user' AND $condition AND p.`cod_per`=d.`cod_per` AND d.`cod_formul`='5' ORDER BY d.`cod_dev` DESC LIMIT $part ,7");
    $rqt->execute();
...
<div class="widget-box">
    <div class="widget-box">
        <div><input type="text" id="nsousdtd"   class="span4" placeholder="Recherche" value="<?php echo $rech;?>"/>
            &nbsp;&nbsp;
            ...
            while ($row_res=$rqt->fetch()){  ?>
                  ...
                  <?php }else { ?>
                    ...
                    <td>&nbsp;
                        ...
                        <a onClick="vdevoy('<?php echo $row_res['cod_dev'];?>','<?php echo $row_res['dat_eff'];?>','assvoygrp.php')" title="Valider"><img  src="img/icons/ok.png"/></a>&nbsp;&nbsp;&nbsp;
                       ...
                       <a onClick="Afficher('<?php echo crypte($row_res['cod_dev']);?>','assvoygrp.php')" title="Assures"><img  src="img/icons/icon_7.png"/></a>&nbsp;&nbsp;&nbsp;

                        <a onClick="ddev('prod','assvoygrp.php','<?php echo $row_res['cod_dev'];?>')" title="Supprimer"><img  src="img/icons/supp.png"/></a>
                    </td>
                </tr>
            <?php } ?>
            </tbody>
