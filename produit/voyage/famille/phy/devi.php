...
if ( isset($_REQUEST['cod']) && isset($_REQUEST['option']) && isset($_REQUEST['duree']) && isset($_REQUEST['pay'])
    ...
    //echo "select * from souscripteurw where cod_par = '$codsous' ";
    $rqtaffich->execute();
    ...
    if ($nb_assur == $cpt) {  
       // $pnga_global = ($pnga_global/$nb_assur) *2.5; MAPFE
        ...
        `cod_formul`, `cod_dt`,`cod_cpl`, `dat_eff`, `dat_ech`, `cap1`, `cap2`, `cap3`, `p1`, `p2`, `p3`, `pn`, `pt`, `bool`, `etat`, `cod_sous`, `taux_int`, `diff_pay`,`com`) VALUES ('','$datesys','$cod_tar','1','$cod_per','$option ','$code_zone','$pay','4',
        '2','2','$dateffet','$datech','0','0','0','$pnga_global','$pngb_global','0','$prime_nette','$prime_total','0','0','$codsous','','','$com')");
        $rqtinsert->execute();
        echo "1";
    } else { echo $cpt;
        echo "2";
