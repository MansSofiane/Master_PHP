...
//echo "SELECT s.*, d.*,p.lib_pays,p.cod_zone, o.lib_opt  FROM `souscripteurw` as s, `policew` as d, `pays` as p, `option` as o WHERE s.cod_sous=d.cod_sous and d.cod_pays=p.cod_pays and d.cod_opt=o.cod_opt  and d.cod_pol='".$row."'";
...
//Carte d'assistance	
        /*    $pdf->Image('../img/carte.png', 10, 204, 190);
            ...
            $pdf->SetXY(40, 224);
            $pdf->Cell(60, 6, '' . $row_user['agence'] . '.' . substr($row_sous['dat_val'], 0, 4) . '.00.18.2.1.' . str_pad((int)$row, '5', "0", STR_PAD_LEFT) . '', '0', '0', 'C');
            $pdf->SetXY(40, 233);
            if ($row_sous['rp_sous'] == 0) {
