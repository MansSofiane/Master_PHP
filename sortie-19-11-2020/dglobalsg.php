...
// Instanciation de la classe derivee
    ...
    //requete
        ...
        if ($agence == '0')
        {
            //la production de la DRE en question code de la DRE est $user;
...
and p.cod_sous=s.cod_sous and s.id_user=u.id_user  and u.id_par='$user' and v.cod_pol= p.cod_pol ) as table1
...
from avenantw as v,policew as p, dtimbre as d,cpolice as c ,souscripteurw as s,utilisateurs as u,produit as pr
where DATE_FORMAT(v.`dat_val`,'%Y-%m-%d') between '$date1' and '$date2'  and v.cod_dt=d.cod_dt and v.cod_cpl=c.cod_cpl and v.cod_prod=pr.cod_prod
and p.cod_sous=s.cod_sous and s.id_user=u.id_user and u.agence='$agence' and v.cod_pol= p.cod_pol ) as table1
group by table1.cod_prod,table1.agence
order by table1.cod_prod
...
// Instanciation de la classe derivee
    ...
    $pdf->Cell(100,10,'AgenceN°: '.'Direction Régionale','1','0','C');$pdf->Cell(90,10,'Produit: Tous les produits ','1','0','C');$pdf->Cell(90,10,'Code produit: ','1','1','C');
    $pdf->Ln();
...
//Reporting Polices
    ...
    $pdf->Cell(40,10,''.number_format($totalg, 2,',',' ').'','1','0','R');$pdf->Ln(10);
