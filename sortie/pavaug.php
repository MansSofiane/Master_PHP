...
$query_sous =$bdd->prepare("SELECT s.*,v.*,o.lib_opt, p.lib_pays,d.sequence as sequence2,d.dat_val as dat_valp,f.lib_formul FROM `souscripteurw` as s, `policew` as d,`avenantw` as v, `option` as o, `pays` as p,formule f WHERE  f.cod_formul=v.cod_formul and  s.cod_sous=d.cod_sous and d.cod_pol=v.cod_pol and v.cod_opt=o.cod_opt and p.cod_pays=v.cod_pays and v.cod_av='".$row."';");
$query_sous->execute();
...
while ($row_sous=$query_sous->fetch()) {
    ...
    while ($row_user = $query_ann->fetch()) {
        $pdf->Cell(190, 8, "Avenant d'augmentation de durée", '0', '0', 'L');
        $pdf->Ln();
        $pdf->SetFont('Arial', 'B', 12);
...
// Voyage
        ...
        $pdf->Cell(55, 5, "" . $row_sous['lib_formul'] . "", '1', '0', 'C');
        $pdf->Ln();
        $pdf->Cell(40, 5, 'Nouvelle date effet', '1', '0', 'L', '1');
        $pdf->Cell(55, 5, "" .date("d/m/Y", strtotime( $row_sous['dat_eff'] )). "", '1', '0', 'C');

        $pdf->Cell(40, 5, 'Nouvelle date echeance', '1', '0', 'L', '1');
