...
$query_ann->execute();
    ...
    while ($row_user = $query_ann->fetch()) {
        $pdf->Cell(190, 8, "Avenant de modification du duree", '0', '0', 'L');
        $pdf->Ln();
        $pdf->SetFont('Arial', 'B', 12);
        $pdf->Cell(190, 8, 'Police Num ' . $row_user['agence'] . '.' . substr($row_sous['dat_valp'], 0, 4) . '.10.18.2.1.' . str_pad((int)$row_sous['sequence2'], '5', "0", STR_PAD_LEFT) . '', '0', '0', 'L');
        $pdf->Ln();
        $pdf->Cell(190, 8, 'Avenant Num ' . $row_user['agence'] . '.' . substr($row_sous['dat_val'], 0, 4) . '.' . $row_sous['lib_mpay'] . '.18.2.1.' . str_pad((int)$row_sous['sequence'], '5', "0", STR_PAD_LEFT) . '', '0', '0', 'L');
        $pdf->Ln();
...
//Le R�seau
        ...
        $pdf->Ln();
        $pdf->Cell(40, 5, 'Telephone', '1', '0', 'L', '1');
...
// Le Tarif !!!!!
        ...
        $pdf->Cell(0, 6, "Il n'est pas autrement deroge aux autres clauses et conditions de la police de base a laquelle le present avenant sera annexe pour en faire partie integrante.", 0, 0, 'C');
        $pdf->Ln(2);
        $pdf->Ln(2);
        $pdf->Ln(5);
        ...
        $pdf->Ln();
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(60, 5, "procedure de la mention Lu et approuve", '0', '0', 'C');
        $pdf->Ln();
        $pdf->Ln(20);
...
// **********************************************
        ...
        $pdf->Cell(190, 10, 'Liste des Assures ', '1', '1', 'C', '1');
        $pdf->SetTextColor(0, 0, 0);
        $pdf->SetFillColor(221, 221, 221);
        $pdf->SetFont('Arial', 'B', 8);
...
//$row_assu = $connection->enr_actuel();
        $pdf->Cell(100, 5, 'Nom et Prenom', '1', '0', 'C', '1');
        $pdf->Cell(45, 5, 'Passport Num', '1', '0', 'C', '1');
