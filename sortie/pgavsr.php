...
$query_sous =$bdd->prepare("SELECT s.*,v.*,o.lib_opt, p.lib_pays,d.sequence as sequence2,d.dat_val as dat_valp,c.mtt_cpl,t.mtt_dt FROM `souscripteurw` as s, `policew` as d,`avenantw` as v, `option` as o, `pays` as p,cpolice as c, dtimbre as t WHERE s.cod_sous=d.cod_sous and d.cod_pol=v.cod_pol and v.cod_opt=o.cod_opt and p.cod_pays=v.cod_pays and v.cod_cpl=c.cod_cpl and v.cod_dt=t.cod_dt and v.cod_av='".$row."';");
$query_sous->execute();
